 ...
 * Description: Implementation of complex shader                               *
 * Created:     3 nov 2021                                                     *
...
char* ComplexShader::writeInitialization(int input_mode)
{
    ...
    case Z_INPUT:
    {
        ...
        sprintf(str_initialization, "vec2 z = vec2(re0, im0);\n"
                                    "vec2 pz = z;\n"
                                    "vec2 c = vec2(0, 0);\n"
                                    "if (drawing_mode == 0)\n"
                                    "    c = vec2(re0, im0);\n"
    ...
    case XY_INPUT:
    {
        ...
        sprintf(str_initialization, "float x = re0;\n"
                                    "float y = im0;\n"
                                    "vec2 pz = vec2(x, y);\n"
                                    "float cx = 0;\n"
                                    "float cy = 0;\n"
...
int ComplexShader::Tree2GLSL(const Tree<CalcData>& node, int input_mode, char* str_cur) const
{
    ...
    case CalcData::FUNCTION:
    {
        ...
        int err = Tree2GLSL(node.branches[0], input_mode, str_cur + strlen(str_cur));
        if (err)
            return err;
    ...
    case CalcData::OPERATOR:
    {
        ...
        {
            int err = Tree2GLSL(node.branches[1], input_mode, str_cur + strlen(str_cur));
            if (err)
                return err;
            sprintf(str_cur + strlen(str_cur), ", ");
        }
