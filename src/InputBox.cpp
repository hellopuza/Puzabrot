...
InputBox::InputBox () :
    box_pos_ ({ 0.0F, 0.0F }), box_color_ (sf::Color (128, 128, 128)), text_color_ (sf::Color::White),
    font_size_ (20.0F)
{
...
void InputBox::draw (sf::RenderWindow& window)
{
    ...
    if (output_text_.getString ().isEmpty ())
        ...
        box_size_ = sf::Vector2f (
            1.5F * input_box.getSize ().y + (input_box.getSize ().x > 0.56F * font_size_ * output_text_size ?
                                                 input_box.getSize ().x :
