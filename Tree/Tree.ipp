...
{
    if (dump_name == nullptr) return -1;
    const char* const DOT_FILE_NAME = "graph.dot";

    std::ofstream dump_file(DOT_FILE_NAME);
    if (! dump_file.is_open()) return -1;

    dump_file << "digraph G{\n" " rankdir = HR;\n" " node[shape=box];\n";

    dump(dump_file);

    dump_file << "\tlabelloc=\"t\";" "\tlabel=\"" << dump_name << "\"; }\n";
...
{
    dump_file << "\t\"" << this << "\"[shape = box, style = filled, color = black, fillcolor = lightskyblue, label = \"" << data << "\"]\n";
